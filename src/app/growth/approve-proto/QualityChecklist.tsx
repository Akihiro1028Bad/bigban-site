...
import type { QualityCheck, QualityLevel } from "./draftQuality";
import { IconCheck, IconChevronDown, IconX } from "./icons";

const TONE: Record<QualityLevel, { color: string; bg: string; label: string }> = {
...
const ORDER: QualityLevel[] = ["block", "warn", "ok"];
              ...
              {sorted.map((c) => (
                <div key={c.id} className="flex items-start gap-2.5">
                  ...
                  <span className="min-w-0">
                    <span className="text-[12.5px]" style={{ color: c.level === "ok" ? "var(--p-text-3)" : "var(--p-text)" }}>
                      {c.label}
                    </span>
                    {c.detail && (
