...
describe("ApproveClient 合言葉画面", () => {
  ...
  it("施策の一覧に種別 chip と category を出し、判断根拠は詳細で見る(#226/#227/#P5a)", async () => {
    ...
    // カードをクリックすると右詳細に施策名の見出しが出る(view 切替後は lane を取り直す)。
    const laneBack = screen.getByRole("region", { name: "施策レーン" });
    await userEvent.click(within(laneBack).getByText("市川ページ"));
    expect(await screen.findByRole("heading", { name: "市川ページ" })).toBeInTheDocument();
  });

  it("subtitle が無い施策でも一覧に出せる(#P5a)", async () => {
    mockFetchSequence({
    ...
    // subtitle 空 → 種別は article へフォールバックし「記事」chip が出る(カード内に限定して照合)。
    const card = within(lane).getByText("A").closest("button") as HTMLElement;
    ...
    mockFetchSequence(
      {
        json: {
          success: true,
          items: [ideaItem(), ideaItem({ id: "i2", title: "他カード" })],
        },
      },
      { ok: false, status: 502, json: { success: false, error: "保存エラー" } },
      { json: { success: true, updated: 1 } }
...
describe("ApproveClient 施策の手動追加(#255)", () => {
  it("施策を追加すると一覧の先頭に承認待ちで増える", async () => {
    ...
    // #P5a: 作成は ProposalView の「手動で追加」→ ProposalFormModal(dialog)経由。
    await userEvent.click(screen.getByRole("button", { name: /手動で追加/ }));
    const modal = await screen.findByRole("dialog", { name: "施策を追加" });
    await userEvent.type(within(modal).getByLabelText("施策名 *"), "手動の施策");
    await userEvent.click(within(modal).getByRole("button", { name: "施策を追加" }));
...
describe("ApproveClient タブ分割/ソート(#242/#90)", () => {
  it("施策タブはレーン降順、記事タブは段階列に分離して表示する(#107/#119)", async () => {
    ...
    // #P5a: master-detail の一覧(左ペイン)に優先度降順で並ぶ。
    const titles = within(lane).getAllByText(/スコア施策/).map((el) => el.textContent);
...
describe("ApproveClient パイプライン盤(#107)", () => {
  ...
  it("状態グループのヘッダに件数を表示する(#107/#P5a)", async () => {
    mockFetchSequence({
      json: {
        success: true,
        items: [
...
describe("ApproveClient 公開タイミングの明示(#237)", () => {
  ...
  it("完了表示に公開タイミングの注記を含む", async () => {
    ...
    // #P5a: 施策の承認は master-detail の詳細フッター(承認ボタン)から行う。
    const lane = screen.getByRole("region", { name: "施策レーン" });
...
describe("ApproveClient 空状態/完了(#236)", () => {
  ...
  it("全件処理すると完了メッセージを表示する", async () => {
    ...
    // #P5a: 施策の承認は master-detail の詳細フッターから行う。
    const lane = screen.getByRole("region", { name: "施策レーン" });
    await userEvent.click(within(lane).getByText("市川ページ"));
    await userEvent.click(await screen.findByRole("button", { name: "承認: 市川ページ" }));
  ...
  it("一部未処理なら完了メッセージは出ない", async () => {
    ...
    // #P5a: 施策の承認は master-detail の詳細フッターから行う。1件だけ承認する。
    const lane = screen.getByRole("region", { name: "施策レーン" });
    await userEvent.click(within(lane).getByText("市川ページ"));
    await userEvent.click(await screen.findByRole("button", { name: "承認: 市川ページ" }));
...
describe("ApproveClient 施策 master-detail(#275/#P5a)", () => {
  // #P5a: 施策 view は proto ProposalView(左一覧＋右詳細)へ移植。行はクリックで右詳細を開き、
  // 承認/却下は詳細フッターに出す(記事の DetailPanel ダイアログではない)。
  it("一覧は高密度化し、数値根拠は一覧に出さない(#275)", async () => {
    mockFetchSequence({ json: { success: true, items: [proposalItem()] } });
    render(<ApproveClient />);
    await login();
    await screen.findByText("市川ページ");
    expect(screen.queryByText("詳細を見る")).not.toBeInTheDocument();
    // 数値根拠(details)は一覧に出さない。
    expect(screen.queryByText("優先度スコア")).not.toBeInTheDocument();
    // 行はクリックで開く(専用の「詳細:」ボタンは持たない)。
    const lane = screen.getByRole("region", { name: "施策レーン" });
    expect(within(lane).getByText("市川ページ")).toBeInTheDocument();
  });

  it("行クリックで右詳細を開き、モバイルの『施策一覧』で閉じる", async () => {
    mockFetchSequence({ json: { success: true, items: [proposalItem()] } });
    render(<ApproveClient />);
    await login();
    await screen.findByText("市川ページ");
    ...
    await userEvent.click(within(lane).getByText("市川ページ"));
    expect(await screen.findByRole("heading", { name: "市川ページ" })).toBeInTheDocument();
    // モバイル用「施策一覧へ戻る」で狭幅の詳細を閉じる(lg 以上は両ペイン維持)。
    await userEvent.click(screen.getByRole("button", { name: "施策一覧へ戻る" }));
    // 詳細ヘッダ(見出し)は lg 以上では残るが、モバイルペイン制御で戻ってもクラッシュしない。
    expect(screen.getByRole("region", { name: "施策レーン" })).toBeInTheDocument();
  });

  it("詳細フッターから承認するとその場で保存する", async () => {
    const fn = mockFetchSequence(
      { json: { success: true, items: [proposalItem()] } },
      { json: { success: true, updated: 1 } }
    );
  ...
  it("詳細フッターから却下もできる", async () => {
    ...
    const lane = screen.getByRole("region", { name: "施策レーン" });
    await userEvent.click(within(lane).getByText("市川ページ"));
    await userEvent.click(await screen.findByRole("button", { name: "却下: 市川ページ" }));

    // 却下フッター(未処理に戻す)へ移る。
    expect(await screen.findByRole("button", { name: "未処理に戻す" })).toBeInTheDocument();
    expect(JSON.parse(fn.mock.calls[1][1].body)).toEqual({
      decisions: [{ id: "p1", decision: "却下" }],
    });
  });

  it("却下後は『未処理に戻す』で pending へ戻せる(#P5a)", async () => {
    const fn = mockFetchSequence(
      { json: { success: true, items: [proposalItem()] } },
      { json: { success: true, updated: 1 } },
      { json: { success: true, updated: 1 } }
    ...
    await userEvent.click(within(lane).getByText("市川ページ"));
    await userEvent.click(await screen.findByRole("button", { name: "却下: 市川ページ" }));
    await userEvent.click(await screen.findByRole("button", { name: "未処理に戻す" }));

    // pending へ戻り、再び承認ボタンが出る。
    expect(await screen.findByRole("button", { name: "承認: 市川ページ" })).toBeInTheDocument();
    // 3回目の呼び出しは未処理へ戻す decision。
    expect(JSON.parse(fn.mock.calls[2][1].body)).toEqual({
  ...
  it("施策の詳細にも準備中プレースホルダは無い(#124)", async () => {
    mockFetchSequence({ json: { success: true, items: [proposalItem()] } });
    render(<ApproveClient />);
    await login();
    await screen.findByText("市川ページ");
    const lane = screen.getByRole("region", { name: "施策レーン" });
    await userEvent.click(within(lane).getByText("市川ページ"));
    // ProposalDetailBody は準備中の下書き生成/AI壁打ちプレースホルダを持たない。
    await screen.findByRole("heading", { name: "市川ページ" });
    expect(screen.queryByRole("button", { name: "下書きを生成" })).not.toBeInTheDocument();
    expect(screen.queryByLabelText("AI壁打ち（準備中）")).not.toBeInTheDocument();
  });

  it("details も subtitle も無い施策でも詳細を開ける", async () => {
    mockFetchSequence({
      json: { success: true, items: [{ id: "p1", kind: "proposal", title: "A", subtitle: "" }] },
    });
    render(<ApproveClient />);
    ...
    await selectView(/施策/);
    const lane = await screen.findByRole("region", { name: "施策レーン" });
    await userEvent.click(within(lane).getByText("A"));
    // subtitle 空 → article 派生・hypothesis 無しでも右詳細ヘッダ(見出し)は開ける。
...
describe("ApproveClient 記事2ペイン master-detail(#proto P3a)", () => {
  it("記事を選択すると右ペインに詳細が出て、モバイルの『← 一覧』で選択解除して閉じる", async () => {
    ...
    await userEvent.click(screen.getByRole("button", { name: "選択記事" }));
  ...
  it("詳細パネル内の『記事一覧へ戻る』(onBack)でも選択解除できる", async () => {
    mockFetchSequence({ json: { success: true, items: [ideaItem({ title: "選択記事" })] } });
    render(<ApproveClient />);
    await login();
    await screen.findByText("選択記事");
    await userEvent.click(screen.getByRole("button", { name: "選択記事" }));
    const dialog = await screen.findByRole("region", { name: "詳細: 選択記事" });
    // DetailPanel 自身の戻る(onBack=setActiveId(null))で選択解除する。
    await userEvent.click(within(dialog).getByRole("button", { name: "記事一覧へ戻る" }));
    expect(screen.queryByRole("region", { name: "詳細: 選択記事" })).not.toBeInTheDocument();
  });

  it("詳細パネルの却下(onReject=decideFromPanel)で保存しパネルを閉じる", async () => {
    const fn = mockFetchSequence(
      { json: { success: true, items: [ideaItem({ title: "選択記事" })] } },
      { json: { success: true, updated: 1 } },
    );
    render(<ApproveClient />);
    await login();
    ...
    await userEvent.click(screen.getByRole("button", { name: "選択記事" }));
    const dialog = await screen.findByRole("region", { name: "詳細: 選択記事" });
    // DetailPanel の却下ボタン → decideFromPanel(item,"却下") → 保存してパネルを閉じる。
    await userEvent.click(within(dialog).getByRole("button", { name: "却下" }));
    expect(await screen.findByText("記事を選択すると詳細が表示されます")).toBeInTheDocument();
    expect(JSON.parse(fn.mock.calls[1][1].body)).toEqual({
      decisions: [{ id: "i1", decision: "却下" }],
    });
  });
...
describe("ApproveClient 構成案修正(#42/#53)", () => {
  ...
  it("構成案クラスタの誌面タブで ArtboardView を表示する", async () => {
    ...
    const dialog = await openIdeaPanel();
    await userEvent.click(within(dialog).getByRole("tab", { name: "構成案" }));
    await userEvent.click(within(dialog).getByRole("tab", { name: "誌面" }));

    expect(within(dialog).getByRole("button", { name: "アイキャッチを開く" })).toBeInTheDocument();
    expect(within(dialog).getAllByText("コート図").length).toBeGreaterThan(0);
  });

  it("誌面で画像スロットを保存すると /revise/edit に textSpec 付き新記法を送る", async () => {
    const fn = mockFetchSequence(
      { json: { success: true, items: [ideaItem({ outline: "## A\n説明\n\n## B\n別説明" })] } },
      { json: { success: true } },
      {
        json: {
          ...
          items: [
            ideaItem({
              outline:
                "## A\n説明\n[画像:手順・フロー図: 入店から予約までの流れ | 文字: 予約→来店→プレー]\n\n## B\n別説明",
            }),
          ],
        },
      }
    ...
    await userEvent.click(within(dialog).getByRole("tab", { name: "構成案" }));
    await userEvent.click(within(dialog).getByRole("tab", { name: "誌面" }));
    await userEvent.click(within(dialog).getByRole("button", { name: "画像を置く: A" }));
    await userEvent.click(within(dialog).getAllByText("フロー図")[0]);
    const description = within(dialog).getAllByLabelText("画像の説明")[0];
    await userEvent.clear(description);
    await userEvent.type(description, "入店から予約までの流れ");
    await userEvent.type(within(dialog).getAllByLabelText("図に入れる文字・数値")[0], "予約→来店→プレー");
    await userEvent.click(within(dialog).getAllByRole("button", { name: "画像指示を保存" })[0]);

    expect(await within(dialog).findByText("入店から予約までの流れ")).toBeInTheDocument();
    expect(within(dialog).getByText("フロー図")).toBeInTheDocument();
    const post = fn.mock.calls[1];
    expect(post[0]).toBe("/api/growth/revise/edit");
    expect(JSON.parse(post[1].body)).toEqual({
      pageId: "i1",
  ...
  it("誌面保存が409なら route の専用メッセージを表示し、エディタは閉じない", async () => {
    mockFetchSequence(
      { json: { success: true, items: [ideaItem({ outline: "## A" })] } },
      { ok: false, status: 409, json: { success: false, error: "x" } }
    );
    ...
    await userEvent.click(within(dialog).getByRole("tab", { name: "誌面" }));
    await userEvent.click(within(dialog).getByRole("button", { name: "画像を置く: A" }));
    await userEvent.type(within(dialog).getAllByLabelText("画像の説明")[0], "図");
    await userEvent.click(within(dialog).getAllByRole("button", { name: "画像指示を保存" })[0]);
    expect(
      (await within(dialog).findAllByText("この記事はAI修正処理中です。完了後に編集してください。"))
        .length
    ).toBeGreaterThan(0);
  ...
  it("相談ドロワーでは SectionImages の代わりに誌面タブへのリンクを表示する", async () => {
    ...
    const dialog = await openIdeaPanel();
    const drawer = await openConsultDrawer(dialog);
    expect(within(drawer).queryByRole("button", { name: "画像を追加: A" })).not.toBeInTheDocument();

    await userEvent.click(within(drawer).getByRole("button", { name: "誌面タブで画像を編集" }));
    expect(await within(dialog).findByRole("button", { name: "画像を置く: A" })).toBeInTheDocument();
  });
});
...
describe("ApproveClient タイトルのAI修正(#139 B)", () => {
  ...
  it("提示中はタイトルの新旧比較を表示し、反映できる", async () => {
    ...
    render(<ApproveClient />);
    await login();
    await userEvent.click(await screen.findByRole("button", { name: "元タイトル" }));
...
describe("ApproveClient 下書きプレビュー(#75)", () => {
  ...
  it("施策(proposal)では下書きプレビューを出さない", async () => {
    ...
    await login();
    await screen.findByText("市川ページ");
    // #P5a: 施策詳細は ProposalDetailBody(下書きプレビュー/タブを持たない)。
    const lane = screen.getByRole("region", { name: "施策レーン" });
    await userEvent.click(within(lane).getByText("市川ページ"));
    await screen.findByRole("heading", { name: "市川ページ" });
...
describe("ApproveClient 下書き手動編集(#77)", () => {
  ...
  it("エディタのメディア画像挿入はメディア選択でエディタ本文だけを更新する", async () => {
    const fn = mockFetchSequence(
      { json: { success: true, items: [ideaItem({ contentId: "g-abc" })] } },
      {
        json: {
      ...
      { json: { success: true, media: [{ url: "https://images.microcms-assets.io/editor-pick.png" }] } },
    );
    render(<ApproveClient />);
    await login();
    await userEvent.click(await screen.findByRole("button", { name: "猛暑記事" }));
    ...
    await userEvent.click(within(getWorkspace()).getByRole("button", { name: "画像メニュー: メディアから挿入" }));
    ...
    expect(fn.mock.calls.find((c) => String(c[0]) === "/api/growth/draft/edit")).toBeUndefined();
  });

  it("エディタのメディア画像挿入モーダルは閉じられる", async () => {
    mockFetchSequence(
      { json: { success: true, items: [ideaItem({ contentId: "g-abc" })] } },
      {
        json: {
    ...
    await userEvent.click(within(getWorkspace()).getByRole("button", { name: "画像メニュー: メディアから挿入" }));
    const media = await screen.findByRole("dialog", { name: "メディアライブラリ: 猛暑記事" });
    await userEvent.click(within(media).getByRole("button", { name: "閉じる" }));
  ...
  it("エディタ内の保存済み画像差し替えはローカル置換だけを実行する", async () => {
    vi.mocked(replacePreservedImageSrc).mockClear();
    const fn = mockFetchSequence(
      { json: { success: true, items: [ideaItem({ contentId: "g-abc" })] } },
      {
        json: {
          ...
          draft: {
            title: "T",
            displayMode: "html",
            bodyHtml: '<figure><img src="https://images.microcms-assets.io/assets/a/body.png" alt="図"></figure>',
            body: "",
          },
        },
      },
      { json: { success: true, media: [{ url: "https://images.microcms-assets.io/editor-replace.png" }] } },
    );
    render(<ApproveClient />);
    await login();
    await userEvent.click(await screen.findByRole("button", { name: "猛暑記事" }));
    ...
    await userEvent.click(within(getWorkspace()).getByRole("button", { name: "保存済み画像を差し替え" }));
    const media = await screen.findByRole("dialog", { name: "メディアライブラリ: 猛暑記事" });
    await userEvent.click(await within(media).findByRole("button", { name: "この画像を挿入" }));

    expect(replacePreservedImageSrc).toHaveBeenCalledWith(
      expect.anything(),
      "https://images.microcms-assets.io/assets/a/body.png",
      "https://images.microcms-assets.io/editor-replace.png",
    );
    expect(fn.mock.calls.find((c) => String(c[0]) === "/api/growth/draft/body-image")).toBeUndefined();
  });

  it("エディタ内の保存済み画像差し替えモーダルは閉じられる", async () => {
    mockFetchSequence(
      { json: { success: true, items: [ideaItem({ contentId: "g-abc" })] } },
      {
        json: {
  ...
  it("エディタのAI画像挿入モーダルは閉じられる", async () => {
    mockFetchSequence(
      { json: { success: true, items: [ideaItem({ contentId: "g-abc" })] } },
      {
        json: {
  ...
  it("エディタ内の保存済み画像AI再生成はモーダルを閉じるとPOSTしない", async () => {
    const fn = mockFetchSequence(
      { json: { success: true, items: [ideaItem({ contentId: "g-abc" })] } },
      {
        json: {
    ...
    await userEvent.click(within(getWorkspace()).getByRole("button", { name: "保存済み画像をAIで再生成" }));
    const modal = await screen.findByRole("dialog", { name: "本文画像をAIで再生成: 猛暑記事" });
    await userEvent.click(within(modal).getByRole("button", { name: "閉じる" }));
  ...
  it("エディタ内の保存済み画像AI再生成は未保存変更があれば保存を促す", async () => {
    const fn = mockFetchSequence(
      { json: { success: true, items: [ideaItem({ contentId: "g-abc" })] } },
      {
        json: {
    ...
    const dialog = await screen.findByRole("region", { name: "詳細: 猛暑記事" });
    await userEvent.click(await within(dialog).findByRole("button", { name: "下書きを編集" }));
    await userEvent.type(within(getWorkspace()).getByLabelText("本文エディタ"), "追記");

    await userEvent.click(within(getWorkspace()).getByRole("button", { name: "保存済み画像をAIで再生成" }));

    expect(await screen.findByText("先に保存してください")).toBeInTheDocument();
    expect(fn.mock.calls.find((c) => String(c[0]) === "/api/growth/body-image/regen")).toBeUndefined();
  });
...
describe("ApproveClient 操作性(#109)", () => {
  // #P5a: 密度/キーボード/一括選択は行内 UI(BoardCard/BoardList)を持つ記事(approve view)で検証する。
  // 施策 view は proto ProposalView(master-detail)へ移植され、行チェックボックスや行フォーカスを持たない。
  // #proto P1: 密度トグル UI は未移植(P6 予定)。既定(comfortable)は行に space-y-2 を適用する。
  it("既定の表示密度(comfortable)では記事行に space-y-2 を適用する", async () => {
    flags.authEnabled = false;
    mockFetchSequence({ json: { success: true, items: [ideaItem()] } });
    render(<ApproveClient />);
    await screen.findByText("猛暑記事");
    const list = screen.getByRole("region", { name: "記事リスト" });
    expect(within(list).getByRole("listitem").className).toContain("space-y-2");
  });
  ...
  it("密度は localStorage から復元する(compact→space-y-1)", async () => {
    ...
    await screen.findByText("猛暑記事");
    const list = screen.getByRole("region", { name: "記事リスト" });
    expect(within(list).getByRole("listitem").className).toContain("space-y-1");
  ...
  it("一括選択して一括承認できる", async () => {
    ...
    mockFetchSequence(
      {
        json: {
          success: true,
          items: [ideaItem(), ideaItem({ id: "i2", title: "他記事" })],
        },
      },
      { json: { success: true, updated: 1 } },
      { json: { success: true, updated: 1 } }
    ...
    render(<ApproveClient />);
    await screen.findByText("猛暑記事");
    await userEvent.click(screen.getByRole("checkbox", { name: "一括選択: 猛暑記事" }));
    await userEvent.click(screen.getByRole("checkbox", { name: "一括選択: 他記事" }));
  ...
  it("入力欄にフォーカス中の単一キーは抑止する", async () => {
    ...
    render(<ApproveClient />);
    await screen.findByText("猛暑記事");
    const checkbox = screen.getByRole("checkbox", { name: "一括選択: 猛暑記事" });
  ...
  it("1/2/3 で詳細タブを構成案/プレビュー/素材へ切り替える", async () => {
    ...
    fireEvent.keyDown(document.body, { key: "e" });
    const dialog = await screen.findByRole("region", { name: "詳細: 猛暑記事" });
    const clusterTabs = within(dialog).getByRole("tablist", { name: "セクション" });
    ...
    await waitFor(() =>
      expect(within(clusterTabs).getByRole("tab", { name: /構成案/ })).toHaveAttribute(
        "aria-selected",
        "true"
      )
    );
...
describe("ApproveClient タブ分離/ルーティング(#119)", () => {
  ...
  it("パレットから施策へジャンプすると施策 view へ自動切替して詳細を開く", async () => {
    ...
    // #P5a: 施策 view(ProposalView)の右詳細に施策名の見出しが出る(記事の dialog ではない)。
    expect(await screen.findByRole("heading", { name: "施策X" })).toBeInTheDocument();
    expect(railButton(/施策/)).toHaveAttribute("aria-current", "page");
  });
});
...
describe("ApproveClient 詳細パネルのレイアウト(#127)", () => {
  ...
  it("施策の詳細はモーダル化せず master-detail の右ペインに出す(#P5a)", async () => {
    mockFetchSequence({ json: { success: true, items: [proposalItem()] } });
    render(<ApproveClient />);
    await login();
    await screen.findByText("市川ページ");
    const lane = screen.getByRole("region", { name: "施策レーン" });
    await userEvent.click(within(lane).getByText("市川ページ"));
    // #P5a: 施策詳細は右ペイン内(ProposalView)。全画面モーダル(dialog/aria-modal/fixed)は使わない。
    expect(await screen.findByRole("heading", { name: "市川ページ" })).toBeInTheDocument();
    expect(screen.queryByRole("dialog", { name: "詳細: 市川ページ" })).not.toBeInTheDocument();
    // 背景オーバーレイ(閉じるボタン)も無い。
    expect(screen.queryByRole("button", { name: "オーバーレイを閉じる" })).not.toBeInTheDocument();
...
describe("ApproveClient 詳細パネルのリッチ化(#124)", () => {
  ...
  it("生成メモ(subtitle)が空の記事は既定文をプロンプト面に出す", async () => {
    mockFetchSequence({
      json: { success: true, items: [ideaItem({ subtitle: "" })] },
    });
    render(<ApproveClient />);
    await login();
    await userEvent.click(await screen.findByRole("button", { name: "猛暑記事" }));
    const dialog = await screen.findByRole("region", { name: "詳細: 猛暑記事" });
    // 2段タブ: 「素材」クラスタ→「プロンプト・参照」リーフへ。subtitle 空 → 既定の生成メモ文が出る
    // (prompt の || フォールバック)。
    await userEvent.click(within(dialog).getByRole("tab", { name: /素材/ }));
    await userEvent.click(within(dialog).getByRole("tab", { name: /プロンプト・参照/ }));
    expect(within(dialog).getByText("この記事の生成メモはまだありません。")).toBeInTheDocument();
  });
...
describe("ApproveClient 連続レビュー＋キーボード(#130)", () => {
  ...
  async function openFirst() {
    ...
    render(<ApproveClient />);
    await login();
    await userEvent.click(await screen.findByRole("button", { name: "記事1" }));
  ...
  it("未処理でない記事(生成待ち)では a/r は無効", async () => {
    ...
    await selectView(/記事/);
    await userEvent.click(await screen.findByRole("button", { name: "生成待ち記事" }));
...
describe("ApproveClient 詳細パネル各タブの結線(#proto P3b)", () => {
  ...
  async function openBodyImages(...after: Array<{ json?: unknown; ok?: boolean; status?: number }>) {
    ...
    await userEvent.click(await screen.findByRole("button", { name: "猛暑記事" }));
    const dialog = await screen.findByRole("region", { name: "詳細: 猛暑記事" });
    await within(dialog).findByRole("tab", { name: /素材/ });
    await gotoImages(dialog);
    return { fn, dialog };
  }
  ...
  it("画像タブ: ＋画像を追加モーダルは閉じられる", async () => {
    const { dialog } = await openInsertImages();
    await userEvent.click(within(dialog).getByRole("button", { name: "＋ 画像を追加" }));
    const insert = await screen.findByRole("dialog", { name: "本文画像を追加: 猛暑記事" });
    await userEvent.click(within(insert).getByRole("button", { name: "閉じる" }));
    await waitFor(() =>
      expect(screen.queryByRole("dialog", { name: "本文画像を追加: 猛暑記事" })).not.toBeInTheDocument(),
    );
  });

  it("画像タブ: 下書き本文HTMLが欠落していても＋画像を追加を開ける", async () => {
    mockFetchSequence(
      { json: { success: true, items: [ideaItem({ contentId: "g-abc" })] } },
      {
        json: {
          success: true,
          exists: true,
          draft: { title: "T", displayMode: "html", body: "" },
        },
      },
    );
    render(<ApproveClient />);
    ...
    expect(await screen.findByRole("dialog", { name: "本文画像を追加: 猛暑記事" })).toBeInTheDocument();
  });

  it("画像タブ: メディアから挿入モーダルは閉じられる", async () => {
    const { dialog } = await openInsertImages(
      { json: { success: true, media: [{ url: INSERT_PICK }] } },
    );
    await userEvent.click(within(dialog).getByRole("button", { name: "＋ 画像を追加" }));
    const insert = await screen.findByRole("dialog", { name: "本文画像を追加: 猛暑記事" });
    await userEvent.click(within(insert).getByRole("button", { name: "メディアから挿入" }));
  ...
  it("画像タブ: メディアから挿入の保存失敗はエラートーストを出す", async () => {
    const { dialog } = await openInsertImages(
      { json: { success: true, media: [{ url: INSERT_PICK }] } },
      { ok: false, status: 500, json: { success: false, error: "保存NG" } },
    );
    await userEvent.click(within(dialog).getByRole("button", { name: "＋ 画像を追加" }));
    const insert = await screen.findByRole("dialog", { name: "本文画像を追加: 猛暑記事" });
    await userEvent.click(within(insert).getByRole("button", { name: "メディアから挿入" }));
    ...
    expect(await screen.findByText("本文画像を挿入できませんでした。")).toBeInTheDocument();
  });
  ...
  it("画像タブ: ＋画像を追加→AIで生成は保存失敗なら再生成依頼しない", async () => {
    const randomUUID = vi.spyOn(crypto, "randomUUID").mockReturnValue("123e4567-e89b-12d3-a456-426614174000");
    const { fn, dialog } = await openInsertImages(
      { ok: false, status: 500, json: { success: false, error: "保存NG" } },
    );
  ...
  it("画像タブ: ＋画像を追加→AIで生成モーダルは閉じられる", async () => {
    const { dialog } = await openInsertImages();
    await userEvent.click(within(dialog).getByRole("button", { name: "＋ 画像を追加" }));
    const insert = await screen.findByRole("dialog", { name: "本文画像を追加: 猛暑記事" });
    await userEvent.click(within(insert).getByRole("button", { name: "AIで生成" }));
    ...
    const regen = await screen.findByRole("dialog", { name: "本文画像をAIで再生成: 猛暑記事" });
    await userEvent.click(within(regen).getByRole("button", { name: "閉じる" }));
