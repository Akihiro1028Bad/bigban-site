...
describe("detailBadge", () => {
  it("記事は実効段階のラベルと色テーマを返す", () => {
    ...
    // 色テーマは段階に応じて変わる
    expect(detailBadge({ id: "i", kind: "idea", stage: "proposed" }).theme.header).toContain("blue");
  });

  it("記事は楽観的承認で『生成待ち』へ前進する", () => {
    const badge = detailBadge({ id: "i", kind: "idea", stage: "proposed" }, "承認");
    expect(badge.label).toBe("生成待ち");
    expect(badge.theme.header).toContain("amber");
  });
  ...
  it("施策の色は状態で変わる(未処理=blue/承認=teal/却下=gray)", () => {
    expect(detailBadge({ id: "p", kind: "proposal", stage: "untouched" }).theme.header).toContain("blue");
    expect(detailBadge({ id: "p", kind: "proposal", stage: "untouched" }, "承認").theme.header).toContain("teal");
    expect(detailBadge({ id: "p", kind: "proposal", stage: "untouched" }, "却下").theme.header).toContain("gray");
  });
});
