...
describe("LeftRail", () => {
  it("5 view を nav/button で出し、アクティブに aria-current=page", () => {
    render(<LeftRail view="approve" articleCount={2} proposalCount={1} queueReadyCount={3} onChange={vi.fn()} />);
  ...
  it("クリックで onChange(該当 view)", async () => {
    ...
    render(<LeftRail view="approve" articleCount={0} proposalCount={0} queueReadyCount={0} onChange={onChange} />);
  ...
  it("バッジ 0 のときはバッジを描画しない", () => {
    render(<LeftRail view="approve" articleCount={0} proposalCount={0} queueReadyCount={0} onChange={vi.fn()} />);
