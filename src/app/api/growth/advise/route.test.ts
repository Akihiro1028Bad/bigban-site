...
function postReq(token: string | null, body: unknown): Request {
  const url = new URL("http://localhost/api/growth/advise");
...
function page(opts: { status?: string; contentId?: string } = {}) {
  ...
  if (opts.status) properties["アドバイスステータス"] = { select: { name: opts.status } };
...
describe("POST /api/growth/advise", () => {
  it("依頼(指示・依頼中・依頼時刻)を書き込む", async () => {
    ...
    const res = await POST(postReq(null, { pageId: PAGE_ID, instruction: "見た目も" }));
    ...
    expect(p["アドバイス指示"].rich_text).toEqual([{ text: { content: "見た目も" } }]);
    expect(p["アドバイスステータス"]).toEqual({ select: { name: "依頼中" } });
  });

  it("指示なしでも依頼できる", async () => {
    vi.mocked(getPage).mockResolvedValue(page({ contentId: "g-abc" }));
    vi.mocked(updatePageProps).mockResolvedValue(PAGE_ID);
    const res = await POST(postReq(null, { pageId: PAGE_ID }));
    expect(res.status).toBe(200);
    const [, props] = vi.mocked(updatePageProps).mock.calls[0];
    expect((props as Record<string, { rich_text?: unknown }>)["アドバイス指示"].rich_text).toEqual([]);
  });
  ...
  it("下書き未作成(contentId空)は 400", async () => {
  ...
  it.each(["依頼中", "処理中", "提示中"])("既に %s なら 409", async (status) => {
  ...
  it("Notion 取得失敗は 502", async () => {
    vi.mocked(getPage).mockRejectedValue(new Error("down"));
    const res = await POST(postReq(null, { pageId: PAGE_ID }));
