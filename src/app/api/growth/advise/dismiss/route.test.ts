...
function postReq(token: string | null, body: unknown): Request {
  const url = new URL("http://localhost/api/growth/advise/dismiss");
...
describe("POST /api/growth/advise/dismiss", () => {
  it("クリア(なし)プロパティを書き込む", async () => {
    ...
    expect((props as Record<string, { select?: { name: string } }>)["アドバイスステータス"]).toEqual({
      select: { name: "なし" },
    });
  });
