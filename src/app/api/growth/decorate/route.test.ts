...
function postReq(token: string | null, body: unknown): Request {
  const url = new URL("http://localhost/api/growth/decorate");
...
function page(opts: { status?: string; contentId?: string } = {}) {
  ...
  if (opts.status) properties["装飾ステータス"] = { select: { name: opts.status } };
...
describe("POST /api/growth/decorate", () => {
  it("依頼(指示・依頼中・依頼時刻)を書き込む", async () => {
    ...
    const res = await POST(postReq(null, { pageId: PAGE_ID, instruction: "強調を" }));
    ...
    expect(p["装飾指示"].rich_text).toEqual([{ text: { content: "強調を" } }]);
    expect(p["装飾ステータス"]).toEqual({ select: { name: "依頼中" } });
  });
  ...
  it("指示なしでも依頼できる", async () => {
    ...
    expect(res.status).toBe(200);
    const [, props] = vi.mocked(updatePageProps).mock.calls[0];
    expect((props as Record<string, { rich_text?: unknown }>)["装飾指示"].rich_text).toEqual([]);
  });
