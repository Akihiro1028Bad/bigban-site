 ...
 *
 * POST { pageId }: 記事を提案中へ戻し、Notion の下書きリンク(下書きID/下書きプレビューキー)を空にする。
 * 用途は構成案・タイトルのやり直し。戻して直して再承認すると、下書きモードが冪等PUTで下書きを作り直す。
 *
 * - microCMS には一切触らない(Notion 書き込みのみ)。本文HTML等のミラーは contentId 空で非表示になるため残置。
...
import { buildRevertProps, isNotionPageId } from "@/lib/growth/approve";
...
export async function POST(request: Request): Promise<Response> {
  ...
  try {
    ...
    await updatePageProps(pageId, buildRevertProps(), options);
  } catch (error) {
