...
export default function HyroxFacility() {
  ...
  /* istanbul ignore next -- @preserve FACILITY_READY=false の間は本番UIを描画しない。公開時に true へ切替＋テスト追加 */
  const body = FACILITY_READY ? (
    <>
      <motion.p
      ...
      >
        ...
        >
          ...
          {NODES.map((node) => (
            <g key={node.key}>
              <circle cx={node.x} cy={node.y} r={27} className="fill-accent" />
              <text
                x={node.x}
                y={node.y}
                textAnchor="middle"
