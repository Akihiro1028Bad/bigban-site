/**
 * 画像指示の純ロジック(#proto からの本番移植・再設計・DOM非依存)。純関数移植。
 * 画像指示 persist は P3b でセッション state に縮約(AD5-2)。
 *
...
export function suggestActions(section: ImageOutlineSection): string[] {
...
export function resolveAction(section: ImageOutlineSection, inst?: ImageInstruction): string {
  ...
  // suggestActions は必ず1件以上返す(FALLBACK_ACTIONS を末尾に足すため)。
  // ?? は noUncheckedIndexedAccess 由来の型安全ガードで実行時には到達しない防御分岐。
  /* istanbul ignore next -- @preserve: 型安全用の到達不能フォールバック */
 ...
 */
export function recommendOff(section: ImageOutlineSection, index: number, total: number): boolean {
  const text = `${section.heading} ${section.summary ?? ""}`;
...
export function imagePlanSummary(outline: ImageOutlineSection[]): { planned: number; specified: number } {
  let planned = 0;
  let specified = 0;
