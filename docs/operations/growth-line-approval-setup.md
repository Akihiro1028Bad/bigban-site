...
> ⚠️ 現状、合言葉認証は **一旦オフ**(`src/config/featureFlags.ts` の `APPROVE_AUTH_ENABLED = false`)。
> ゲートは出ず、承認/施策追加 API も token 検証をスキップする(URL を知る人は誰でも操作可能・一時措置)。
> **復元するには `APPROVE_AUTH_ENABLED` を `true` に戻すだけ**(1行・env 設定不要・再デプロイ)。以降の `APPROVE_SECRET` 設定は復元後に有効になる。
