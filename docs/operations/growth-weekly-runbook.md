...
監査行は品質ゲートの確認済みfact復元には混ぜない。旧方式の fact marker が公開本文に残っている場合は HTML安全性チェックが block するため、手動で削って公開せず下書きを再生成する。
...
4. コードが本文と fact の値照合から逆引きした根拠台帳と、カテゴリ、画像配置、Notion更新先を含む投入specを組み立てる。キャッシュは`.growth-tmp/drafts/<pageId>/`に保存し、リサーチは24時間、執筆は入力・プロンプト・モデル設定が同じ限り再利用する。「構成からやり直す」は新しい生成attemptとしてキャッシュを分け、同じattempt内の障害再試行だけを再利用する。
