...
承認画面で記事ネタ案の**構成案(見出しアウトライン)に行ごとのコメント**を付けて「修正を依頼」すると、
常時稼働PCが拾って `claude` で構成案を直し、ユーザーが**提示→確認して反映**する。プル型(Web は Notion に
依頼を書くだけ／PC が5分間隔で拾う)。推論はサブスクの `claude` なので **API 追加課金なし**。
...
1. **依頼(あなた)**: 承認画面の記事詳細パネルで構成案の各行にコメント→「修正を依頼」。
2. **処理(PC・自動)**: 5分間隔のタスク([Windows セットアップ](./growth-windows-setup.md) 6.5)が `npm run growth:revise-loop` を起動。
   `claude` が構成案を §13/§14/§15 と facility-context に沿って直し、「提示中」にして **LINE に通知**。最大5分待ち。
3. **確認(あなた)**: 通知の承認画面URLを開き、元の構成案と修正案を見比べて「反映」or「やり直し(再コメント)」。
