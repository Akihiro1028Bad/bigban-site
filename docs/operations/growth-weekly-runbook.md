...
> 方針: **アイキャッチの画調は [growth-article-style.md](./growth-article-style.md) §9 のアートディレクションに固定**(明るいエアリー×ブランド配色)。記事ごとに画風をバラつかせない。被写体だけ記事内容に合わせて選ぶ。

1. **アートディレクター** ― §9 の固定スタイルを前提に、記事に合う被写体を選ぶ
   - **アイキャッチ(必須)**: §9 の画調(明るいエアリーな実写・白〜ライト基調・ブランドのブルー＋黄ライムの差し色)で生成。記事ごとに必ず1枚・**文字入れは避ける**(記事カード/OGP で日本語が崩れて見えるのを防ぐ)。被写体は §9 の記事タイプ別テンプレから選ぶ(記事内容と矛盾しない題材。例: 屋内施設の記事に屋外コートを出さない)
   - **本文画像(任意)**: 説明を助ける箇所にのみ。無理に入れない。**文字入れOK**(図解・ラベル・キャプション等)。図解は §9 の画調に縛られない
   - レイアウトや配色の整えは**デザイナー**
2. **画像生成** ― `openai-image-gen` スキル(gpt-image-2)で、**§9 の「固定スタイルブロック + 記事タイプ別の被写体1行」**でプロンプトを組み立てる(即興でゼロから書かない)。アイキャッチは横長(16:9・1536x1024)、文字なし、**`--quality high` 固定**。本文画像は文字入れ可(図解等)
