...
Writer は金額・日付・時刻・数値単位・統計・限定的な健康断定・確認済み固有名詞を含む主張へ、句読点直前（表はセル終端直前）に `<!--FACT:fact-id-->` を付ける。複数根拠はカンマ区切りにする。オーケストレーターは `htmlparser2` で同じ `p` / `li` / `th` / `td` の直前1文だけを束縛し、ResearchPacket と atom が一致しない本文をキャッシュ・投入しない。

検証後の本文から marker は除去され、根拠台帳へ fact ID、抜粋、見出し経路、要素位置、公開前再確認フラグが保存される。旧spec・旧checkpointには marker がないため、再開経路は従来どおり扱う。
