...
| プロパティ | 型 | 用途 |
|---|---|---|
| `根拠台帳` | rich_text | 執筆時の source_ledger(主張・出典・発行年・確度・可否・理由)を **publish-draft が自動書き込み**する(読者非公開・監査用)。1行1エントリのプレーンテキスト。投入スペックの `sourceLedger` を検証・整形して保存。 |

- **任意プロパティ・欠落耐性**: **未追加でも投入は成功する**(台帳込みで更新→プロパティ起因の失敗時は台帳抜きでリトライし、警告を stderr/LINE に出す=沈黙させない)。
- `sourceLedger` 欠落/空でも従来どおり投入成功。不正項目は警告付きで除外(投入全体は落とさない)。
