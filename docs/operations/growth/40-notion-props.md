...
- **予約件数は手入力しない**: 既存の `予約件数` プロパティは互換性のため残してもよいが、`growth:existing` は読み取らない。実予約は予約CSV、または将来の予約メール自動取得データを正本とし、欠損時に手入力へフォールバックしない。
- **4項目とも任意・欠落耐性**: 未入力なら週次は「手入力データなし」で従来動作(推測しない)。`growth:existing` が「オーナー手入力」節で読み、週次がSNS・口コミの成功指標に接地させる。
