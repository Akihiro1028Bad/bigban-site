...
> CSV取り込み、GA4 Data APIによる集計、分析画面、施策提案はこの手順の対象外。
...
flowchart LR
  ...
  B -->|reservation_click| C["LaBOLA<br/>情報入力"]
...
`reservation_click` にはクリック位置を表す `location` が付くため、公式サイトを離れる時点ではピックルボール、HYROXエリア、HYROXレッスンを区別できる。
...
通常予約系のイベントだけでは、LaBOLA内部のピックルボールとHYROXエリアを分離できない。両者の内訳が必要な場合は、公式サイト側の `reservation_click` の `location` を使う。動的な予約種別をLaBOLAから渡せることが確認できるまでは、推測のパラメータを追加しない。
...
ここまでで本PRの計測追加は完了とする。GA4 Data APIでの取得、ファネル集計、CSVとの照合、分析画面への表示は別PRで行う。
