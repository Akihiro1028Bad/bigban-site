...
`reservation_click` は従来の予約意図指標として維持する。週次のLaBOLA導線集計には、GA4カスタムディメンションの手動登録を必要としない専用流入イベントを使う。RESERVAへの移動では専用流入イベントを送らない。
...
- [ ] RESERVAへの移動では `labola_entry_*` が着弾しない
