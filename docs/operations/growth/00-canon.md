...
- **予約公開**: 人間が再認証後に日時を指定した時点で、対象記事へ将来の公開権限を付与する。非AIの決定的workerである `npm run growth:publish-due` が到来分だけを再検証して公開する。予約解除は公開権限の撤回であり、解除後にworkerは公開しない。
...
| (非AI・決定的worker) | `growth:publish-due` | 人間が予約公開を許可した到来分だけを再検証して公開(#H24) | [50-publish-metrics.md](50-publish-metrics.md) |
