...
- 起動しておくだけで、各ループを**間隔ごとに自動巡回**（pull系=既定1分・publish-due=5分・stall-check=15分）。依頼が無いループは一瞬で空振りするので、依頼は**押してから最大1〜2分**で拾われます。
...
|---|---|---|---|
| `growth-revise` | `npm run growth:revise-loop` | 5 分 | 構成案/タイトルの修正依頼を拾う（#40）。多重起動は lockfile で防止、1 日上限 `GROWTH_REVISE_DAILY_CAP`（既定 50）。 |
