...
| **計測ループ** `metrics-cli.ts` → `metrics.ts articlePagePath` | ✅ | `mediaOf(page)` を `articlePagePath(slug, locale, media)` に渡し、GA4/GSC の URL セグメントを媒体で解決。 |
...
- `weekly.md`: 記事ネタ案に**両媒体を提案**。`媒体=ニュース` は**告知版S4**(確定事実のみ・doNotWrite 厳守・日時断定禁止・本数別枠)。コラムは従来ルール維持。
