...
- 既知の偽陰性: 終了マーカーは本文全体で見るため、「前回は終了しました」と書きつつ次回を受付中の記事では H1 が付かない（許容）。
...
| 記号 | 条件 | 深刻度 |
|---|---|---|
| S | GA4 に入口セッション（直近7日 + 前7日 が **3 以上**）があり、sitemap の記事 URL に無いパス（上限超過で未取得のものは含まない）。ページが記事として描画されていれば 低、ソフト404・非200 なら 中、接続不能なら 低 | 低〜中 |

- 候補は記事詳細 URL の形（`/columns/<slug>`・`/news/<slug>`・`/en/` 付き。`articleWatch.mjs` の `ARTICLE_URL`）に限る。GA4 の入口パスは部分一致で記事扱いになるため、`/columns/foo/bar` のようなルートに存在しないパスを除く。
- 最低流入は `THRESHOLDS.sMinSessions = 3`（2026-09-15 追加）。実データで、存在しない記事に14日で1セッションしか無いのに「S 中」が毎朝出た。1件の誤リンクや bot で鳴らさない。
- `/en/` で始まるパスは対象外（EN 記事の sitemap 未掲載は既知。2026-09-14 時点）。
- sitemap の上限（`maxArticles = 50`）を超えて未取得になった記事（`overflow`）は **sitemap に載っている**ので S の対象にしない。
- 上限は `THRESHOLDS.maxUnlisted = 10` 本。流入（直近7日 + 前7日）の降順・同数はパス昇順に並べてから先頭10本を取り、超えた分は取得せず `sources.sitemap.error` に「sitemap 外の記事が10本を超えたため未確認: …」として記録する。
...
sitemap 外の記事（S の候補）が非200 だと1本あたり再試行待ち（既定30秒）が入る。並列5・最大10本なので最悪 +60秒。
...
```
{
  ...
  sources: {
    ...
    html:    { ok, fetched, error }                       // 1本でも取得失敗（観測不能含む）なら ok:false
                                                          // （sitemap の記事のみ。sitemap 外の失敗は S の detail.page で表す）
  },
  articles: [{
...
`detectHttpFlag` / `detectSitemapFlag` は `http.observed === "ok"` のとき `parsed` が必須（CLI は 200 のとき必ず html を渡す）。それ以外の `observed` では `parsed` を読まないので `null` でよい。
...
- `sources.sitemap.error` は `ok` が true でも読む（`maxUnlisted` 超過のメッセージは `ok` を false にしないため）。sitemap に無いのに流入がある記事は `alerts` の `S` として出る（§3.4）。EN ページは対象外（sitemap 未掲載が既知）。`sources.sitemap.error` に「sitemap 外の記事が10本を超えたため未確認: …」が入っていれば、その件数を1行添える。
...
| ③ | sitemap に記事の詳細 URL が ja/en とも載っている | 実装時に本番 sitemap で確認 | 載っていない記事は監視対象外。sitemap に無く GA4 に流入がある記事はそれ自体を信号（S）として通知する（§3.4） |
