...
- **具体設計**: `growth:existing` の出力に**公開済み記事の成績サマリ**（記事タイプ別の成功率・判定済み母数・未判定率・観測条件・検索反応）を含める。成功本数や率による自動ランキングは行わず固定記事タイプ順で供給し、weekly は条件が揃わない型の比較を保留する。判定済み3本未満は探索中として、本命1件＋補欠1件＋探索1件の判断材料にする。却下再提案の件数と本命/探索の配分を週次完了報告に出す。→ Issue F / #286。
- **受け入れ基準**:
  - [ ] `existing` 出力に記事タイプ別成績が含まれる（純ロジックのテスト）。
  - [x] weekly が成功率・判定済み母数・未判定率・観測条件をセットで参照し、成功本数だけで優先しない。
  - [x] 判定済み3本未満を探索中とし、5/20と2/2の上下関係を自動生成しない。
  - [x] GA4/GSCの未計測と実測0を分け、検索反応を予約成果や「勝ちクエリ」と呼ばない。
  - [x] 通常記事提案を本命1件＋補欠1件＋探索1件とし、同型の生きた案・観測中記事があれば探索を積み増さない。
  - [ ] 却下再提案件数が週次完了報告に出る。
