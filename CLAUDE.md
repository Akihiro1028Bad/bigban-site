...
- **リードで結論先出し**: 導入は1〜2文で結論から。excerpt も結論を先頭に置くプレーンテキスト120〜160字(description/OGP/JSON-LDに使われる)。
- **カニバリ回避**: 新記事は既存公開記事・起票済みネタと検索意図が実質同一なら書かない。角度違いは「既存との差分」を構成案に明記できるときだけ可。
- **出典の記録**: 記事で使った出典・確認済み事実は Notion 記事ネタ案の「一次情報メモ」欄に残す(次回の改稿・関連記事で再利用するため)。
