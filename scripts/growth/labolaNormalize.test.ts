...
describe("buildCanonical", () => {
  ...
  it("プログラムと予約不可を正準化し、Noneを除外する", () => {
    ...
    expect(bundle.programs).toHaveLength(1); expect(bundle.blockedSlots).toEqual([{ date: "2026-07-17", start: "10:00", end: "11:00", space: "A" }]); expect(JSON.stringify(bundle.blockedSlots)).not.toContain("清掃"); expect(bundle.meta.counts).toMatchObject({ program: 1, blocked: 1 });
  });
