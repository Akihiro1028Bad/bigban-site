...
it("欠落と基準入力の半分未満の行数を検出する", () => {
  ...
  const output = dataHealth({ bundle: bundle as never, previousSnapshot: null, baselineInputs: previousSnapshot.meta.inputs, funnel: null, kpi: null, current: { start: "", end: "" }, prior: { start: "", end: "" }, todayYmd: "" });
...
it("前回入力が無い・前回行数が0・半減していない場合は行数急減を出さない", () => {
  ...
  expect(dataHealth({ bundle: bundle as never, previousSnapshot, baselineInputs: previousSnapshot.meta.inputs, funnel: null, kpi: null, current: { start: "", end: "" }, prior: { start: "", end: "" }, todayYmd: "" })).toEqual([]);
});

it("欠落種別を並び替えたIDにして内容変更を新規通知できる", () => {
  const bundle = { meta: { missingSections: ["sales", "customer"], counts: {} } };
  const output = dataHealth({ bundle: bundle as never, previousSnapshot: null, baselineInputs: null, funnel: null, kpi: null, current: { start: "", end: "" }, prior: { start: "", end: "" }, todayYmd: "" });
