...
it("各期間nが10未満なら変化を出さない", () => {
  ...
  expect(selfRateChange({ bundle: bundle as never, previousSnapshot: null, baselineInputs: null, funnel: null, current: { start: "2026-07-13", end: "2026-07-19" }, prior: { start: "", end: "" }, todayYmd: "" })).toEqual([]);
});
...
it("28日窓のWilson区間が重ならないセルフ予約率をnoticeにする", () => {
  ...
  const bundle = { reservations, customers: [], salesDaily: [], programs: [], blockedSlots: [], remarks: [], meta: { coverage: { start: "2026-05-01", end: "2026-07-19" } } };
  expect(selfRateChange({ bundle: bundle as never, previousSnapshot: null, baselineInputs: null, funnel: null, current: { start: "2026-07-13", end: "2026-07-19" }, prior: { start: "", end: "" }, todayYmd: "" })[0]).toMatchObject({ id: "d3:selfRate:2026-07-13", severity: "notice", label: "有意" });
});
