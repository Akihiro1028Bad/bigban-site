...
it("備考本文をbodyに含めず現在週のものだけ通知する", () => {
  ...
  const output = newRemarks({ bundle: bundle as never, previousSnapshot: null, baselineInputs: null, funnel: null, kpi: null, current: { start: "2026-07-13", end: "2026-07-19" }, prior: { start: "", end: "" }, todayYmd: "" });
...
it("予約が見つからない備考と週外の備考を除外する", () => {
  ...
  expect(newRemarks({ bundle: bundle as never, previousSnapshot: null, baselineInputs: null, funnel: null, kpi: null, current: { start: "2026-07-13", end: "2026-07-19" }, prior: { start: "", end: "" }, todayYmd: "" })).toEqual([]);
});
