...
export const firstSeenWard: Detector = (context) => {
  ...
  const previous = new Set(context.previousSnapshot.catalog.wards.map((entry) => entry.ward));
  return wardCounts(context.bundle)
    .filter((entry) => entry.ward !== "不明" && entry.customers >= 1 && !previous.has(entry.ward))
