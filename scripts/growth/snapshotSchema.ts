/** 予約分析スナップショットの共有スキーマ。 */
import { z } from "zod";
import { coverageSchema, isoDateTimeSchema, ymdSchema } from "./dateSchemas";
...
export const snapshotSchema = z.object({
  ...
  analysis: z.object({
    referenceYmd: ymdSchema,
    currentWeek: z.object({ start: ymdSchema, end: ymdSchema }),
  }),
  meta: z.object({
  ...
  catalog: z.object({
    ...
    programFills: z.array(z.object({ name: z.string(), heldOn: ymdSchema, start: z.string(), capacity: z.number().nullable(), reserved: z.number(), fillRate: z.number().nullable() })).optional(),
    unpaidAging: z.object({ count: z.number(), amount: z.number(), buckets: z.array(z.object({ label: z.enum(["0-7日", "8-14日", "15日以上"]), count: z.number(), amount: z.number() })) }).nullable().optional(),
