 ...
 *
 *   npm run growth:gen-body-image -- --style <mascot|minimal|diagram> --description "<説明>" --out <出力パス>
...
import { buildBodyImagePrompt, type BodyImageStyle } from "./body-image";
import { generateImage } from "./eyecatch";
import { defaultFetch } from "./http";

const here = path.dirname(fileURLToPath(import.meta.url));
const DEFAULT_REF = path.join(here, "assets", "mascot-alien.png");
const STYLES: readonly BodyImageStyle[] = ["mascot", "minimal", "diagram"];
...
async function main(): Promise<void> {
  ...
  if (!style || !(STYLES as readonly string[]).includes(style)) {
    throw new Error(`--style は ${STYLES.join("|")} のいずれかを指定してください。`);
  }
  ...
  const prompt = buildBodyImagePrompt(style as BodyImageStyle, description);
  const buf = await generateImage(
    {
      ...
      size,
      quality,
      // mascot のみ参照画像でキャラを保持。minimal/diagram は参照なし(publish-draft と同方針)。
