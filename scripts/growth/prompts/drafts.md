...
- 目標品質のお手本(構成・密度・文体を真似る・**文章はコピーしない**): `scripts/growth/prompts/examples/example-beginner-local.md`、`scripts/growth/prompts/examples/example-trend.md`、`scripts/growth/prompts/examples/example-scene-first.md`、`scripts/growth/prompts/examples/example-fear-relief.md`、`scripts/growth/prompts/examples/example-asset-rule.md`。**5例はそれぞれ異なる骨格**を示す(番号付き3セクション/初心者ガイド/場面起点・逆説/不安解消型/資産型)。**同一骨格をコピーせず**、記事の主題に合う型を選ぶ(番号付き3セクションだけが唯一の型ではない)。各例冒頭の`【この例で学ぶこと】`ラベルで、その例から盗む技を確認する。
