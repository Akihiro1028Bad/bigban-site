...
2. **執筆チーム(2体)で執筆する**。①執筆担当、②編集者(別エージェント)。各記事について次の順で進める:
   ...
   2-3. **承認済み構成の固定**: 手順1で読んだ現在の `構成案` を執筆構造として固定する。H2/H3のレベル・見出し名・順序を変えず、各見出し下の説明をそのセクションの狙いとして扱う。構成案にない見出しは追加しない。ただし§15の生成条件を満たす場合に限り、末尾へH2 `参考資料`を追加できる。画像指示も元のセクション位置を維持する。お手本から構成を再設計しない。
