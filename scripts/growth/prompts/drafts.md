...
3. **投入スペックをステージする**(エージェントはここまで)。記事ごとに `<output_schema>` の JSON を一時ファイル(例 `.growth-tmp/<slug>.json`)に書く。**この時点では create も画像生成もしない**(投入は手順4のスクリプトに任せ、エージェントが重い処理を背景タスク化して待ちでストールするのを防ぐ=#23)。
   ...
   - **`sourceLedger`(根拠台帳の永続化 #根拠台帳・任意)**: 手順2-2 で作った台帳を spec トップレベルの `sourceLedger` 配列に入れる(読者非公開・監査用に Notion 行へ保存)。**キーは camelCase**(spec の慣習):`claim` / `sourceType` / `source` / `publishedYear`(統計・数値は必須・数値型)/ `confidence` / `usableInArticle`(真偽値)/ `reason`。`not-used`(usableInArticle=false)の行も監査のため含めてよい。台帳が無ければ省略してよい(欠落耐性=投入は従来どおり成功)。手順4のスクリプトが検証し、不正項目は警告付きで除外する。

4. **画像プロンプト設計→投入を同期実行する**: 記事ごとに、まず `npm run growth:image-prompt -- .growth-tmp/<slug>.json` を実行する。承認画面の「画像プロンプト設計」で選んだ独立モデルが、本文を変更せず `eyecatchAction` と `images[].style/description/textSpec` だけを具体化する。コマンドが失敗した場合は投入せず、その記事を保留して報告する。成功後に `npm run growth:publish-draft -- .growth-tmp/<slug>.json` を実行する。このスクリプトが **品質ゲート(`draftQuality`)→ create(冪等PUT)→ アイキャッチ生成(参照画像方式)→ upload → eyecatch添付 → 記事ネタ案DBのステータス更新** を直列・同期で行う(背景タスクにしない・完了を待たない運用にしない)。
...
```json
{
  ...
  "sourceLedger": [
    { "claim": "<本文で使った主張>", "sourceType": "facility-context|official-site|search-result|not-used",
      ...
      "confidence": "high|medium|low", "usableInArticle": true, "reason": "<判断理由>" }
  ],
