...
2. **執筆チーム(2体)で執筆する**。①執筆担当、②編集者(別エージェント)。各記事について次の順で進める:
   ...
   2-4. **編集者ゲート(採点ではなく合否)**: 編集者は次を**すべて**チェックし、**1つでも block 該当があれば下書き化を保留**して理由を報告する(手順4の `publish-draft` は同じ機械ゲートで投入をハードに止めるため、ここで必ず潰す):
       - **block(必須・赤)**: ①§5 の AI 免責文(末尾)があるか ②facility-context との矛盾が無いか(開業時期のズレ等) ③§13 doNotWrite(料金・営業時間・コート面数・所要分)を断定していないか ④本文の主要主張がすべて source_ledger に存在し usable=true か ⑤内部リンク先が実在するか(壊れリンクを書かない・§15) ⑥§11 の数値ゲート(§11-1: 具体性/独自視点/翻訳調・AI 臭を 0〜5 で採点し、いずれかが閾値以下なら block。減点理由には本文の該当箇所を1つ以上引用。採点結果 quality_score を報告に含める) ⑦リードが悩み代弁テンプレ/宣言文になっていないか(§4-1 リードの型) ⑧体験談が供給ソース(§16 ①一次情報メモ ②confirmed ③裏取りリスト)に実在するか。存在しない一場面(創作の体験談)を書いていないか
       - **採点(§11 品質ルブリック)**: 具体性 / 4段構成 / 独自視点 / 翻訳調なし(**§14 の禁止パターン全て**。単一ソース=§14 を参照)/ リンク・数値・タイトル(§15: 外部リンク濫用/不完全URL、未検証・可変数値の断定、キーワード盛りタイトル、内部リンクの検討漏れ)
...
4. **投入を同期実行する**: 記事ごとに `npm run growth:publish-draft -- .growth-tmp/<slug>.json` を実行する。このスクリプトが **品質ゲート(`draftQuality`)→ create(冪等PUT)→ アイキャッチ生成(参照画像方式)→ upload → eyecatch添付 → 記事ネタ案DBのステータス更新** を直列・同期で行う(背景タスクにしない・完了を待たない運用にしない)。
   - **品質ゲート**: 投入直前に `draftQuality` を実行し、**block(AI免責文欠落・§13断定・壊れ内部リンク 等)があれば投入を中断**し、不合格理由を出力・Notion ステータスを保留・**LINE 通知**して終了コード1で終わる(沈黙させない)。手順2-4 を通っていれば通常はここで止まらない。
