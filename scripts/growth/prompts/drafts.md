...
1. **対象記事の取得と着手マーク**: 「記事ネタ案」DB(data source `5adab8b1-f182-4123-b963-9463a2580d4a`)から `ステータス = 承認`(および前回中断で `生成中` のまま残った行 #108)をすべて取得(無ければ「承認済みなし」と報告して終了)。取得時に各行の `媒体` プロパティ(`コラム`/`ニュース`・未追加なら `コラム` 扱い)も読み、手順3の payload 分岐に使う(#media)。
