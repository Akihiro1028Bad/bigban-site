...
describe("施設系接尾辞の固有名詞判定", () => {
  // 捏造検出は維持する: facts に無い施設名は marker 必須のままでなければならない。
  it.each([
    ["船橋市総合体育館の屋内コートで練習できます。", "捏造した体育館名"],
    ["船橋ピックルボールクラブが毎週活動しています。", "捏造したクラブ名"],
    ["西船橋駅から徒歩圏内に専用コートがあります。", "捏造した駅名"],
    ["千葉県ピックルボール協会が講師を派遣しています。", "捏造した協会名"],
  ])("%s は根拠なしに書けない(%s)", (sentence) => {
    expect(() => validate(`<p>${sentence}</p>`, [])).toThrow(/fact markerのない必須主張/);
  });

  // 一般名詞は固有名詞ではないので、marker なしで書けなければならない。
  it.each([
  ...
  ])("%s は一般論として書ける(%s)", (sentence) => {
    expect(() => validate(`<p>${sentence}</p>`, [])).not.toThrow();
  });
