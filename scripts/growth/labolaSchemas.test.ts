...
const header = ["予約番号", "予約内容", "日付", "開始時間", "終了時間", "カテゴリー", "予約ステータス", "受付ステータス", "入金ステータス", "料金プラン", "金額", "支払い方法", "利用人数", "受付日時", "最終更新日時", "予約方法", "顧客タイプ", "会員番号", "名前", "メールアドレス", "郵便番号", "住所", "性別", "生年月日", "職業", "備考"];
const defaultValues: Record<string, string> = { "予約番号": "90", "予約内容": "A:テストコート", "日付": "2026/08/13", "開始時間": "19:00", "終了時間": "21:00", "カテゴリー": "スペース予約", "予約ステータス": "予約完了", "受付ステータス": "受付済", "入金ステータス": "未払い", "料金プラン": "一般価格", "金額": "15960", "支払い方法": "クレジットカード", "利用人数": "", "受付日時": "2026年07月15日 14:19", "最終更新日時": "2026年07月15日 14:19", "予約方法": "スマートフォン版のユーザー", "顧客タイプ": "一般会員", "会員番号": "00901", "名前": "試験太郎", "メールアドレス": "taro@example.com", "郵便番号": "1000001", "住所": "東京都千代田区1-1-1", "性別": "男性", "生年月日": "1990/01/02", "職業": "会社員", "備考": "" };
...
describe("parseProgramRows / parseBlockedSlotRows", () => {
  ...
  it("表示名を予約名より優先してlabelにする", () => {
    const blockedHeader = ["日付", "開始時間", "終了時間", "スペース", "予約名", "表示名"];
    expect(parseBlockedSlotRows([blockedHeader, ["2026/07/17", "09:00", "10:00", "A", "旧名称", "実CSVの表示名"]]).rows[0]).toMatchObject({ label: "実CSVの表示名" });
  });
...
describe("parseYoyakuRows", () => {
  it("実CSVの予約方法と入金ステータスを型付きに変換する", () => { const result = parseYoyakuRows([header, row({ "開始時間": "9:00" })]); expect(result.warnings).toEqual([]); expect(result.rows[0]).toMatchObject({ reservationId: "90", bookedAt: "2026-07-15T14:19:00+09:00", useDate: "2026-08-13", start: "09:00", status: "confirmed", channel: "user_sp", paymentStatus: "未払い", amount: 15960, space: "A:テストコート" }); });
  it("PC版のユーザーをPC経由に写像する", () => expect(parseYoyakuRows([header, row({ "予約方法": "PC版のユーザー" })]).rows[0].channel).toBe("user_pc"));
...
describe("parseCustomerRows", () => {
  ...
  it("全角括弧の会員番号ヘッダーを半角の必須名として照合する", () => {
    const fullWidthHeader = customerHeader.map((name) => name === "会員番号(自動発行)" ? "会員番号（自動発行）" : name);
    expect(parseCustomerRows([fullWidthHeader, ["2026年7月5日", "一般", "001", "試験顧客", "sample@example.com", "", "東京都台東区1-1", "", "", "会社員"]]).rows[0]).toMatchObject({ memberNo: "001" });
  });
...
describe("parseSalesSummaryRows", () => {
  it("見込み行を判定して数値化する", () => { const { rows } = parseSalesSummaryRows([["売上日", "レンタルスペース", "イベント", "会員費", "売上合計"], ["2026年07月15日", "8980", "0", "0", "8980"], ["2026年07月17日(見込み)", "0", "18000", "0", "18000"]]); expect(rows[0]).toEqual({ date: "2026-07-15", isForecast: false, rentalSpace: 8980, event: 0, memberFees: 0, total: 8980 }); expect(rows[1].isForecast).toBe(true); expect(rows[1].date).toBe("2026-07-17"); });
  it("空CSV・必須列不足・不正な金額をエラーにする", () => {
    expect(() => parseSalesSummaryRows([])).toThrow("売上サマリCSVが空");
    expect(() => parseSalesSummaryRows([["売上日"]])).toThrow("必須列");
    expect(() => parseSalesSummaryRows([["売上日", "レンタルスペース", "イベント", "会員費", "売上合計"], ["2026年7月1日", "x", "0", "0", "0"]])).toThrow("金額");
  });
  it("売上額が空欄なら0に補完する", () => {
    const rows = parseSalesSummaryRows([["売上日", "レンタルスペース", "イベント", "会員費", "売上合計"], ["2026/07/15", "", "", "", ""]]).rows;
    expect(rows[0]).toMatchObject({ rentalSpace: 0, event: 0, memberFees: 0, total: 0 });
  });
  it("売上行の欠損セルを行番号付きで拒否する", () => {
    expect(() => parseSalesSummaryRows([["売上日", "レンタルスペース", "イベント", "会員費", "売上合計"], ["2026/07/15"]])).toThrow("2行目: 必須列 レンタルスペース のセルがありません");
  });
});
