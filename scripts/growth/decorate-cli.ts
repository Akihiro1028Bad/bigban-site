 ...
 *   npm run growth:decorate -- peek                          # 依頼中の件数を読み取り専用で出力
...
async function reap(options: NotionApiOptions): Promise<void> {
  ...
  await reapStaleRows(rows, Date.now(), DECORATE_TIMEOUT_MS, async (row) => {
...
async function main(): Promise<void> {
  ...
  switch (command) {
    ...
    default:
      throw new Error("使い方: decorate-cli <reap|peek|next|present|fail> ...");
  }
}
