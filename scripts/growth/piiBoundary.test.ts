...
describe("occupationGroup", () => { it("粗いグループへ丸め、医療職以外の師を医療扱いしない", () => { expect(occupationGroup("会社員")).toBe("会社員"); expect(occupationGroup("鍼灸師")).toBe("医療・施術"); expect(occupationGroup("理学療法士")).toBe("医療・施術"); expect(occupationGroup("学生")).toBe("学生"); expect(occupationGroup("経営者")).toBe("経営・自営"); expect(occupationGroup("パイロット")).toBe("その他"); expect(occupationGroup("教師")).toBe("その他"); expect(occupationGroup("美容師")).toBe("その他"); expect(occupationGroup("")).toBe("不明"); }); });
