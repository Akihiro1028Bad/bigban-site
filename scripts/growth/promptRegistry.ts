...
export const PROMPT_REGISTRY: readonly PromptPhaseMeta[] = [
  ...
  meta({
    path: "scripts/growth/prompts/draft-research.md",
    label: "記事リサーチ",
    group: "実行プロンプト",
    order: 30,
    purpose: "承認済み構成案に必要な重要事実を公式情報から確認する",
    stage: "記事リサーチ",
    whenItRuns: "下書き生成の最初に独立した調査セッションを起動するとき(growth:drafts)",
    sourceKind: "prompt",
  }),
  meta({
    path: "scripts/growth/prompts/draft-write.md",
    label: "記事執筆",
    group: "実行プロンプト",
    order: 31,
    purpose: "確認済みのResearchPacketから記事本文を1回で執筆する",
    stage: "記事執筆",
    whenItRuns: "リサーチ完了後にクリーンな独立セッションで本文を作るとき(growth:drafts)",
    sourceKind: "prompt",
  }),
  meta({
  ...
  meta({
    ...
    group: "文体例",
    order: 10,
    purpose: "トレンド系記事を人が検討するときの参考例",
    stage: "任意参考",
    whenItRuns: "構成検討や人手レビューで必要なときだけ参照(下書き生成には渡さない)",
    sourceKind: "example",
  }),
  meta({
    ...
    group: "文体例",
    order: 20,
    purpose: "初心者×地域系記事を人が検討するときの参考例",
    stage: "任意参考",
    whenItRuns: "構成検討や人手レビューで必要なときだけ参照(下書き生成には渡さない)",
    sourceKind: "example",
  }),
  meta({
    ...
    group: "文体例",
    order: 30,
    purpose: "場面起点・逆説系記事を人が検討するときの参考例",
    stage: "任意参考",
    whenItRuns: "構成検討や人手レビューで必要なときだけ参照(下書き生成には渡さない)",
    sourceKind: "example",
  }),
  ...
  meta({
    ...
    group: "文体例",
    order: 60,
    purpose: "ありがちな事故と修正例を示す人手レビュー用の参考資料",
    stage: "任意アドバイス",
    whenItRuns: "人がアドバイスや本文コメント修正を依頼したときだけ参照",
    sourceKind: "example",
  }),
];
